...
if(move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $target)) {
    ...
    $sql="INSERT INTO uml_chat(uploaded_file, message, date_time, adv_id)
...
<form class="search-form">
  ...
  <tr> 
    ...
    <form method="post" name="formUserReg" id="formUserReg" action="advisor_UML_chat.php" enctype="multipart/form-data" >
    <?php if (isset($errors)) 
        { 
        ?>
	    <p><?php echo $errors; ?></p>
	...
	<tbody>
		...
		$message = mysqli_query($con, "SELECT * FROM uml_chat where adv_id=$user_id");

		 while ($row = mysqli_fetch_array($message)) { ?>
			<tr>
				
                <td class="chat_id"> <?php echo $row['uml_id']; ?> </td>
