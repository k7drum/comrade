...
if(move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $target)) {
    ...
    $sql="INSERT INTO uml_chat(uploaded_file, message, date_time, student_id)
...
<body>
  ...
  <tr> 
    ...
    <form method="post" name="formUserReg" id="formUserReg" action="UML_chat.php" enctype="multipart/form-data" >
    <?php if (isset($errors)) 
        { 
        ?>
	    <p><?php echo $errors; ?></p>
	...
	<tbody>
		<?php 
		// select all UML_chats if page is visited or refreshed
		$message = mysqli_query($con, "SELECT * FROM uml_chat where student_id=$user_id");

		 while ($row = mysqli_fetch_array($message)) { ?>
			<tr>
				
                <td class="uml_id"> <?php echo $row['uml_id']; ?> </td>
                <td class="student_id"> <?php echo $row['student_id']; ?> </td>
                <td class="adv_id"> <?php echo $row['adv_id']; ?> </td>
